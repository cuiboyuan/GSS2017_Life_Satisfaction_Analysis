...
gss_numeric = data.frame(feelings_life = gss_data$feelings_life, marriage = gss_data$marital_status, education=edu, health=health_numeric, income_respondent=IncomeRes_numeric,
...
Marital status (code in raw data:MARSTAT, name in 'gss.csv':marital_status) is a categorical data indicating whether the respondent is:
```{r, echo=FALSE}
group_by(gss_numeric, marriage) %>% summarise() %>%tibble()
```


Education level (code in raw data:EHG3_01B, name in 'gss.csv':education) is a categorical data that indicates the highest education the respondent acquires. For the convenience of modeling, we will transform the data into continuous numeric variable. The transformed variable will range from 1-7, representing from the lowest to the highest education level (we use the order of education level from the original GSS). Below is a reference table:
```{r, echo=FALSE}
...
rank = group_by(gss_numeric, education) %>% summarise()
rank = rank$education
tibble(`Education Level`=edu_name, `Numeric Rank`=rank)
```


Individual income (code in raw data:TTLINCG2, name in 'gss.csv':income_respondent) and family income (code in raw data:FAMINCG2, name in 'gss.csv':income_family) are both categorical data. Individual income indicates the income level of respondent, while family income indicates the total income of respondent's family. Both incomes are before tax and exclude any capital gains or losses. We will transform these two variables to numeric data ranging from 1 to 7 which represent the income level. Below is a reference table shared by both family and respondent income:
```{r, echo=FALSE}
inc_name = c("Less than $25,000","$25,000 to $49,999","$50,000 to $74,999","$75,000 to $99,999","$100,000 to $ 124,999","$125,000 and more")
rank = group_by(gss_numeric, income_respondent) %>% summarise()
rank = rank$income_respondent
tibble(`Income Level`=inc_name, `Numeric Level`=rank)
```


Self rated health (code in raw data:SRH_110, name in 'gss.csv':self_rated_health) is a categorical data that indicates what the respondent think his/her health level is. We will also transform this variable to numeric data with range from 1-5. Below is the reference table:
```{r, echo=FALSE}
health_name = c("Poor","Fair","Good","Very good","Execellent")
rank = group_by(gss_numeric, health) %>% summarise()
rank = rank$health
tibble(`Self Rated Health Level`=health_name, `Numeric Level`=rank)
```


Average working hours (code in raw data:UHW_16GR, name in 'gss.csv':average_hours_worked) is a categorical data that indicates how many hours the respondent works a week on average. We will convert the variable to numeric with range from 1-5. Below is the reference table:
```{r, echo=FALSE}
hrs_name = c("0 hour","0.1 to 29.9 hours","30.0 to 40.0 hours","40.1 to 50.0 hours","50.1 hours and more")
rank = group_by(gss_numeric, work_hours) %>% summarise()
...
summary(gss_numeric$feelings_life)
```
